var trex, trex_running, trex_collided, trex_jump, trex_bend;
var ground, invisibleGround, groundImage;
var birdsGroup, birdImage;
var checkpoint, dead, jump;

var cloudsGroup, cloudImage;
var obstaclesGroup, obstacle1, obstacle2, obstacle3, obstacle4, obstacle5, obstacle6;

var restartimg, restart, gameoverimg, over;
...
function draw() {
  background(255);
  if (frameCount % 3200 > 1600 && gamestate === PLAY) {
    background(0);
  }
  if (score === score + 400 && gamestate === PLAY) {
    background(0);
  }
  if (score <= 400) {
    background(255);
  ...
  if (gamestate === PLAY) {
    ...
    ground.velocityX = -(4 + 10 * score / 200);
    if (ground.x < width / 9) {
      ground.x = ground.width / 2;
    }
